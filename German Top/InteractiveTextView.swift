...
struct InteractiveTextView: View {
    let text: String; var onWordTap: (String) -> Void
    var body: some View {
        let words = text.components(separatedBy: .whitespacesAndNewlines).filter { !$0.isEmpty }.enumerated().map { InteractiveWord(id: $0, content: $1) }
        FlowLayout(spacing: 8) {
            ForEach(words) { word in
                let clean = word.content.trimmingCharacters(in: .punctuationCharacters)
                Button { if !clean.isEmpty { onWordTap(clean) } } label: {
                    Text(word.content).font(.body).foregroundColor(.blue).padding(.horizontal, 4).padding(.vertical, 2).background(Color.blue.opacity(0.1)).cornerRadius(4)
                }.buttonStyle(.plain)
            }
        }
    }
...
struct FlowLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        let width = proposal.width ?? .infinity; var h: CGFloat = 0, x: CGFloat = 0, y: CGFloat = 0, rowH: CGFloat = 0
        for v in subviews {
